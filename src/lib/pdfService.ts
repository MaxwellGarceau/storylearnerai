...
export class PDFService {
   ...
   */
  static async extractText(file: File, maxPages = 10): PDFExtractionPromise {
    try {
      ...
      logger.error('translation', 'PDF processing error', {
        error: err,
        fileName: file.name,
      });
      return {
   ...
   */
  private static mapToPDFTextItem(item: PDFJSTextItem): PDFTextItem {
    return {
      text: item.str,
   ...
   */
  private static reconstructTextWithLineBreaks(items: PDFTextItem[]): string {
    ...
    for (let i = 0; i < sortedItems.length; i++) {
      ...
      if (i < sortedItems.length - 1) {
        ...
        if (nextYDiff <= lineTolerance) {
          ...
          if (gap > 3) {
            // Reduced threshold for more sensitive spacing
            currentLine += ' ';
          } else if (gap > 0) {
  ...
  private static fixPunctuationSpacing(text: string): string {
    ...
    // Note: We skip quote handling to preserve original PDF formatting
    // PDFs often contain malformed quotes that can break regex logic
    // Preserving original whitespace around quotes is more reliable
  ...
  static async getFileInfo(file: File): Promise<PDFFileInfo | null> {
    try {
      ...
      logger.error('translation', 'Error getting PDF file info', {
        error: err,
        fileName: file.name,
      });
      return null;
