...
describe('Walkthrough Button Behavior', () => {
  ...
  let mockStateChangeCallback: ((state: WalkthroughState) => void) | null =
    null;
  ...
  beforeEach(() => {
    ...
    vi.mocked(walkthroughService.getCurrentConfig).mockReturnValue(
      mockMultiStepWalkthrough
    );
    vi.mocked(walkthroughService.getCurrentStep).mockReturnValue(
      mockMultiStepWalkthrough.steps[0]
    );
  });
  ...
  describe('Next Button Navigation', () => {
    it('should show new modal window with next step content when Next button is clicked', async () => {
      ...
      expect(
        screen.getByText('First step of the walkthrough')
      ).toBeInTheDocument();
      ...
      vi.mocked(walkthroughService.nextStep).mockImplementation(() => {
        logger.debug(
          'general',
          '🔧 Service: nextStep() called, updating to step 2'
        );
        const newState = {
          ...initialState,
        ...
        vi.mocked(walkthroughService.getCurrentStep).mockReturnValue(
          mockMultiStepWalkthrough.steps[1]
        );
        ...
        if (mockStateChangeCallback) {
          logger.debug(
            'general',
            '🔧 Service: Notifying subscribers with new state',
            { newState }
          );
          act(() => {
      ...
      logger.debug(
        'general',
        '✅ Modal window is still present after Next click'
      );
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText('Second step of the walkthrough')
        ).toBeInTheDocument();
      });
      ...
      expect(
        screen.queryByText('First step of the walkthrough')
    ...
    it('should advance from step 2 to step 3 when Next button is clicked', async () => {
      ...
      vi.mocked(walkthroughService.nextStep).mockImplementation(() => {
        ...
        vi.mocked(walkthroughService.getCurrentStep).mockReturnValue(
          mockMultiStepWalkthrough.steps[2]
        );
        vi.mocked(walkthroughService.getState).mockReturnValue(newState);
    ...
    it('should complete walkthrough when Next button is clicked on final step', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByTestId('walkthrough-modal')
        ).not.toBeInTheDocument();
      });
    });
  });
    ...
    it('should close modal immediately when X button is clicked', async () => {
      ...
      vi.mocked(walkthroughService.stopWalkthrough).mockImplementation(() => {
        ...
        if (mockStateChangeCallback) {
          logger.debug(
            'general',
            '🔧 X Button: Notifying subscribers modal should close'
          );
          act(() => {
  ...
  describe('Positioning Behavior', () => {
    it('should update positioning when moving between steps with same target element', async () => {
      ...
      vi.mocked(walkthroughService.getState).mockReturnValue(initialState);
      vi.mocked(walkthroughService.getCurrentConfig).mockReturnValue(
        sameTargetWalkthrough
      );
      vi.mocked(walkthroughService.getCurrentStep).mockReturnValue(
        sameTargetWalkthrough.steps[0]
      );
