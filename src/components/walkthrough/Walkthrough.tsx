...
export const Walkthrough: React.FC<WalkthroughProps> = () => {
  ...
  const updateSpotlight = useCallback(
    (forceUpdate = false, elementOverride?: HTMLElement | null) => {
      ...
      if (
        forceUpdate ||
        Math.abs(newPosition.left - currentLeft) > threshold ||
  ...
  useEffect(() => {
    ...
    if (element) {
      ...
      logger.warn(
        'ui',
        `Target element not found: ${currentStep.targetSelector}`
      );
  ...
  useEffect(() => {
    ...
    const scrollableElements = document.querySelectorAll(
      '[data-scrollable], .overflow-auto, .overflow-scroll, .overflow-y-auto, .overflow-y-scroll'
    );
                ...
                <div className='space-y-3 sm:space-y-4'>
                  ...
                  <div className='flex items-center justify-between text-xs text-muted-foreground'>
                    <Badge variant='secondary' className='text-xs'>
                      Step {currentVisibleStepIndex + 1} of{' '}
                      {allVisibleSteps.length}
                    </Badge>
