...
describe('StoryUploadForm', () => {
  ...
  it('renders the form with correct Spanish to English translation context', () => {
    ...
    expect(
      within(container).getByText('Spanish → English')
    ).toBeInTheDocument();
    expect(
      within(container).getByText(/Enter your Spanish story below/)
    ).toBeInTheDocument();

    // Check form elements
    expect(
      within(container).getByRole('textbox', { name: /Spanish Story/i })
    ).toBeInTheDocument();
    expect(
      within(container).getByLabelText(/Target Language/i)
    ).toBeInTheDocument();
    expect(
      within(container).getByLabelText(/Target Difficulty \(CEFR\)/i)
    ).toBeInTheDocument();
    expect(
      within(container).getByRole('button', { name: /Translate Story/i })
    ).toBeInTheDocument();
  });
  ...
  it('allows the user to type in the textarea', () => {
    ...
    fireEvent.change(textArea, {
      target: { value: 'Una historia de ejemplo' },
    });
  ...
  it('triggers onSubmitStory with complete form data when the form is submitted', () => {
    ...
    const { container } = render(
      <StoryUploadForm onSubmitStory={onSubmitStoryMock} />
    );
    const textArea = within(container).getByRole('textbox', {
      name: /Spanish Story/i,
    });
  ...
  it('displays enhanced difficulty options with descriptions', () => {
    ...
    const difficultySelectTrigger = within(container).getByLabelText(
    ...
    expect(
      within(container).getByRole('option', { name: 'A1 (Beginner)' })
    ).toBeInTheDocument();
    expect(
      within(container).getByRole('option', { name: 'A2 (Elementary)' })
    ).toBeInTheDocument();
    expect(
      within(container).getByRole('option', { name: 'B1 (Intermediate)' })
    ).toBeInTheDocument();
    expect(
      within(container).getByRole('option', { name: 'B2 (Upper Intermediate)' })
    ).toBeInTheDocument();
  });
  ...
  it('handles select changes for difficulty', () => {
    ...
    const difficultyOption = within(container).getByRole('option', {
      name: 'B1 (Intermediate)',
    });
    fireEvent.click(difficultyOption);

    expect(
      within(container).getByLabelText('Select difficulty level')
  ...
  it('displays helpful context information', () => {
    ...
    expect(
      within(container).getByText(
        'Write or paste the Spanish story text you wish to translate to English.'
      )
    ).toBeInTheDocument();
    expect(
      within(container).getByText(
