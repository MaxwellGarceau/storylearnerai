...
describe('custom/no-non-localized-text', () => {
  it('allows using t() function', async () => {
    ...
    const messages = result.messages.filter(m => m.ruleId === 'testCustom/no-non-localized-text')
  ...
  it('reports non-localized text in JSX', async () => {
    ...
    expect(messages[0].message).toContain("Non-localized text 'Hello World' found")
  })
  ...
  it('reports non-localized text in JSX attribute', async () => {
    ...
    const messages = result.messages.filter(m => m.ruleId === 'testCustom/no-non-localized-text')
    expect(messages.length).toBe(1)
    expect(messages[0].message).toContain("Non-localized text 'Click me' found")
  })
  ...
  it('reports non-localized text in JSX expression', async () => {
    ...
    const messages = result.messages.filter(m => m.ruleId === 'testCustom/no-non-localized-text')
    expect(messages.length).toBe(1)
    expect(messages[0].message).toContain("Non-localized text 'Welcome to our app' found")
  })
})
